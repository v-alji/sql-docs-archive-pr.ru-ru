...
> [!NOTE]  
>  Длина типа данных `varchar(max)` различается для каждой строки. sp_help возвращает (-1) в качестве длины `varchar(max)` столбца. [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] отображает значение -1 как размер столбца.  
