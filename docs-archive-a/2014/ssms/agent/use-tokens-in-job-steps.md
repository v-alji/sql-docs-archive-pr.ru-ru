...
|**Дата**|Текущая дата (в формате ГГГГММДД).|  
...
|**ТАЙМАУТ**|Текущее время (в формате ЧЧММСС).|  
...
|------------------|--------------------------------|---------------------------------|  
|Управляющие макросы используются|Все токены в заданиях заменяются успешно.|Токены, активированные предупреждениями, не заменяются. Эти токены представляют **собой DBN**, **a – SVR**, **a-Err**, **A-уровень серьезности**, **a-MSG**и **WMI ( *`property`* )**. Другие статические токены заменяются успешно.|  
