...
# <a name="testing-a-filtered-model-basic-data-mining-tutorial"></a>Проверка модели с фильтром (учебник по интеллектуальному анализу данных — начальный уровень)
  Теперь, когда вы определили, что `TM_Decision_Tree` модель является наиболее точной, вы настроите модель так, чтобы она лучше соответствовала потребностям [!INCLUDE[ssSampleDBCoFull](../includes/sssampledbcofull-md.md)] кампании целевой рассылки. В частности, Отделу маркетинга хотелось бы выяснить, есть ли какие-либо различия между двумя сотрудниками и клиентами-женщина. Эти сведения помогут решить, какие журналы следует использовать для рекламы и какие продукты должны быть в своей почте.  
...
3.  Щелкните правой кнопкой мыши `TM_Decision_Tree` модель и выберите **создать модель интеллектуального анализа данных.**  
