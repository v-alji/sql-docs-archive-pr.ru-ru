...
## <a name="xml-serialization"></a>XML-сериализация  
 Определяемые пользователем типы должны поддерживать преобразование из типа данных `xml` и в этот тип данных, подчиняясь соглашению для XML-сериализации. Пространство имен `System.Xml.Serialization` содержит классы, используемые для сериализации объектов в документы или потоки формата XML. По желанию можно реализовать сериализацию `xml` с помощью интерфейса `IXmlSerializable`, предоставляющего настраиваемую XML-сериализацию и десериализацию.  
