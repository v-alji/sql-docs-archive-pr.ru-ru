...
### <a name="example"></a>Пример  
 В следующем примере требуется создать вторую таблицу, содержащую столбец определяемого пользователем типа `Point`, а также столбец `timestamp`. Обе таблицы используются, чтобы показать, как создать пользовательские командные объекты для обновления данных и как выполнить обновление с помощью столбца `timestamp`. Запустите следующие инструкции [!INCLUDE[tsql](../../includes/tsql-md.md)], чтобы создать вторую таблицу и заполнить ее образцами данными.  
