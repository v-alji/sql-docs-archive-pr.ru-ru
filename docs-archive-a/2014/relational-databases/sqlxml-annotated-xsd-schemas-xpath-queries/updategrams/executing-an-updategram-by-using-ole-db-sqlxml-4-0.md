---
title: Исполнение Диаграмма обновления с помощью OLE DB (SQLXML 4,0) | Документация Майкрософт
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "87732454"
---
# <a name="executing-an-updategram-by-using-ole-db-sqlxml-40"></a><span data-ttu-id="ad396-102">Выполнение диаграммы обновления с помощью OLE DB (SQLXML 4.0)</span><span class="sxs-lookup"><span data-stu-id="ad396-102">Executing an Updategram by Using OLE DB (SQLXML 4.0)</span></span>
  <span data-ttu-id="ad396-103">В этом разделе представлен рабочий пример Усинголе DB для выполнения диаграмма обновления.</span><span class="sxs-lookup"><span data-stu-id="ad396-103">This topic provides a working sample of usingOLE DB to execute an updategram.</span></span>  
  
## <a name="using-icommandstream-to-set-an-xml-command"></a><span data-ttu-id="ad396-104">Использование интерфейса ICommandStream для установки XML-команды</span><span class="sxs-lookup"><span data-stu-id="ad396-104">Using ICommandStream to Set an XML Command</span></span>  
 <span data-ttu-id="ad396-105">OLE DB интерфейс Икоммандстреам (версии 2,6 или более поздней) передает команду в виде объекта потока, а не в виде строки.</span><span class="sxs-lookup"><span data-stu-id="ad396-105">The OLE DB (version 2.6 or later) ICommandStream interface passes a command as a stream object, rather than as a string.</span></span>  
  
 <span data-ttu-id="ad396-106">Этот интерфейс позволяет представить команду в любой кодировке, приемлемой для синтаксического XML-анализатора.</span><span class="sxs-lookup"><span data-stu-id="ad396-106">This interface allows a command to be in any encoding that the XML parser understands.</span></span> <span data-ttu-id="ad396-107">Когда вызывается метод ICommand:: Execute, текст команды считывается непосредственно из потока и преобразование не требуется.</span><span class="sxs-lookup"><span data-stu-id="ad396-107">When ICommand::Execute is called, the command text is read out of the stream directly and no conversion is required.</span></span> <span data-ttu-id="ad396-108">Поэтому выполнять команды XML с помощью интерфейса Икоммандстреам более эффективно.</span><span class="sxs-lookup"><span data-stu-id="ad396-108">Therefore, executing XML commands using ICommandStream interface is more efficient.</span></span>  
  
### <a name="setting-xml-as-a-command-using-icommandstream-and-retrieving-the-results-as-an-xml-document"></a><span data-ttu-id="ad396-109">Установка XML как команды с использованием интерфейса ICommandStream и получение результатов как XML-документа</span><span class="sxs-lookup"><span data-stu-id="ad396-109">Setting XML as a Command Using ICommandStream and Retrieving the Results as an XML Document</span></span>  
 <span data-ttu-id="ad396-110">Интерфейс Икоммандстреам можно использовать для установки XML-документов в качестве команды, а результаты можно извлечь в виде XML-документа.</span><span class="sxs-lookup"><span data-stu-id="ad396-110">The ICommandStream interface can be used to set XML documents as a command, and the results can be retrieved as an XML document.</span></span>  
  
#### <a name="executing-templates-with-xpath-queries"></a><span data-ttu-id="ad396-111">Выполнение шаблонов с запросами XPath</span><span class="sxs-lookup"><span data-stu-id="ad396-111">Executing Templates with XPath Queries</span></span>  
 <span data-ttu-id="ad396-112">Следующий XML-шаблон, состоящий из запроса XPath, указывается в виде команды с помощью Икоммандстреам:</span><span class="sxs-lookup"><span data-stu-id="ad396-112">The following XML template consisting of an XPath query is specified as a command using ICommandStream:</span></span>  
...
```  
 ...
 <span data-ttu-id="ad396-113">Запрос XPath в шаблоне выполняется для следующей схемы сопоставления:</span><span class="sxs-lookup"><span data-stu-id="ad396-113">The XPath query in the template is executed against the following mapping schema:</span></span>  
...
```  
 ...
 <span data-ttu-id="ad396-114">Запрос возвращает все элементы работника.</span><span class="sxs-lookup"><span data-stu-id="ad396-114">The query returns all of the employee elements.</span></span> <span data-ttu-id="ad396-115">При использовании сопоставления по умолчанию **\<Person.Contact>** элемент сопоставляется с таблицей Person. Contact в базе данных AdventureWorks.</span><span class="sxs-lookup"><span data-stu-id="ad396-115">With default mapping, the **\<Person.Contact>** element maps to the Person.Contact table in the AdventureWorks database.</span></span>  
  
###### <a name="to-set-xml-as-a-command-and-retrieving-result-as-an-xml-document"></a><span data-ttu-id="ad396-116">Установка XML как команды и получение результата как XML-документа</span><span class="sxs-lookup"><span data-stu-id="ad396-116">To set XML as a command and retrieving result as an XML document</span></span>  
  
1.  <span data-ttu-id="ad396-117">Инициализируйте и установите соединение с базой данных.</span><span class="sxs-lookup"><span data-stu-id="ad396-117">Initialize and establish a connection to the database.</span></span>  
  
2.  <span data-ttu-id="ad396-118">Получите интерфейс Икоммандстреам на ICommand.</span><span class="sxs-lookup"><span data-stu-id="ad396-118">Obtain the ICommandStream interface on ICommand.</span></span>  
  
3.  <span data-ttu-id="ad396-119">Установите необходимые свойства команды.</span><span class="sxs-lookup"><span data-stu-id="ad396-119">Set the necessary command properties.</span></span> <span data-ttu-id="ad396-120">В этом примере специфическое для поставщика свойство SSPROP_STREAM_BASEPATH устанавливается для каталога, в котором хранятся схема сопоставления и файлы шаблонов.</span><span class="sxs-lookup"><span data-stu-id="ad396-120">In this example, the provider-specific property SSPROP_STREAM_BASEPATH is set to the directory where the mapping schema and the template files are stored.</span></span>  
  
4.  <span data-ttu-id="ad396-121">Используйте Икоммандстреам:: Сеткоммандстреам, чтобы указать поток команд.</span><span class="sxs-lookup"><span data-stu-id="ad396-121">Use ICommandStream::SetCommandStream to specify the command stream.</span></span> <span data-ttu-id="ad396-122">В этом примере выполняемый XML-шаблон считывается из файла.</span><span class="sxs-lookup"><span data-stu-id="ad396-122">In this example, the XML template being executed is read from a file.</span></span> <span data-ttu-id="ad396-123">Эта возможность полезна, когда требуется выполнить большие XML-шаблоны.</span><span class="sxs-lookup"><span data-stu-id="ad396-123">This is useful when you want to execute large XML templates.</span></span>  
  
5.  <span data-ttu-id="ad396-124">Выполните команду XML с помощью метода ICommand:: Execute, запросив идентификатор интерфейса IID_ISequentialStream.</span><span class="sxs-lookup"><span data-stu-id="ad396-124">Execute the XML command using ICommand::Execute, requesting the IID_ISequentialStream interface ID.</span></span>  
  
6.  <span data-ttu-id="ad396-125">Обработайте результат.</span><span class="sxs-lookup"><span data-stu-id="ad396-125">Process the result.</span></span> <span data-ttu-id="ad396-126">В этом примере XML, считанный из потока, отображается на экране.</span><span class="sxs-lookup"><span data-stu-id="ad396-126">In this example, the XML read from the stream is displayed on the screen.</span></span>  
...
#### <a name="passing-parameters-to-templates"></a><span data-ttu-id="ad396-127">Передача параметров в шаблоны</span><span class="sxs-lookup"><span data-stu-id="ad396-127">Passing Parameters to Templates</span></span>  
 <span data-ttu-id="ad396-128">В этом примере показано, как можно передать значения параметров в XML-команды.</span><span class="sxs-lookup"><span data-stu-id="ad396-128">This example shows how parameter values can be passed to XML commands.</span></span> <span data-ttu-id="ad396-129">Этот XML-шаблон задан как команда:</span><span class="sxs-lookup"><span data-stu-id="ad396-129">This XML template is specified as a command:</span></span>  
...
```  
 ...
 <span data-ttu-id="ad396-130">Шаблон включает SQL-запрос.</span><span class="sxs-lookup"><span data-stu-id="ad396-130">The template includes an SQL query.</span></span> <span data-ttu-id="ad396-131">Для запроса требуется значение параметра ( @Title ).</span><span class="sxs-lookup"><span data-stu-id="ad396-131">The query requires a value for its parameter (@Title).</span></span> <span data-ttu-id="ad396-132">Если значение параметра не передано, используется значение по умолчанию («Mr.»).</span><span class="sxs-lookup"><span data-stu-id="ad396-132">If no parameter value is passed, the default value ("Mr.") is used.</span></span>  
  
 <span data-ttu-id="ad396-133">При передаче значений параметра в шаблон должны быть указаны как имя, так и значение параметра.</span><span class="sxs-lookup"><span data-stu-id="ad396-133">In passing parameter values to a template, the parameter name and value both must be specified.</span></span>  
  
 <span data-ttu-id="ad396-134">В этом коде:</span><span class="sxs-lookup"><span data-stu-id="ad396-134">This is the code:</span></span>  
