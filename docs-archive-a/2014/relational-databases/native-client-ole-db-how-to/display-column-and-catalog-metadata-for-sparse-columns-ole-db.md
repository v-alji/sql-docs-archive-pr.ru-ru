...
ms.locfileid: "87734057"
---
# <a name="display-column-and-catalog-metadata-for-sparse-columns-ole-db"></a><span data-ttu-id="bd762-102">Отображение метаданных столбца и каталога для разреженных столбцов (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="bd762-102">Display Column and Catalog Metadata for Sparse Columns (OLE DB)</span></span>
  <span data-ttu-id="bd762-103">В этом примере создается таблица, состоящая из трех столбцов: разреженного столбца, столбца, не являющегося разреженным, и столбца, представляющего собой набор столбцов.</span><span class="sxs-lookup"><span data-stu-id="bd762-103">This sample creates a table with three columns: a sparse column, a column that is not a sparse column, and a columnset column.</span></span> <span data-ttu-id="bd762-104">Затем образец отображает флаги OLE DB, показывающие метаданные столбца и каталога для неразреженного столбца и столбца, представляющего собой набор столбцов.</span><span class="sxs-lookup"><span data-stu-id="bd762-104">The sample then displays OLE DB flags showing the column and catalog metadata for the non-sparse column and the columnset column.</span></span>  
  
 <span data-ttu-id="bd762-105">Данный образец работает с [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] или более поздней версией.</span><span class="sxs-lookup"><span data-stu-id="bd762-105">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span> <span data-ttu-id="bd762-106">Дополнительные сведения о разреженных столбцах см. [в разделе Поддержка разреженных столбцов в SQL Server Native Client](../native-client/features/sparse-columns-support-in-sql-server-native-client.md).</span><span class="sxs-lookup"><span data-stu-id="bd762-106">For more information about sparse columns, see [Sparse Columns Support in SQL Server Native Client](../native-client/features/sparse-columns-support-in-sql-server-native-client.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="bd762-107">Пример</span><span class="sxs-lookup"><span data-stu-id="bd762-107">Example</span></span>  
 <span data-ttu-id="bd762-108">Убедитесь, что переменная среды INCLUDE включает каталог, содержащий файл sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="bd762-108">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
