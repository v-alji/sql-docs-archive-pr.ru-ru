...
ms.locfileid: "87658693"
---
# <a name="send-data-to-a-filestream-column-using-isequentialstream-bound-to-icommandtext-parameter-ole-db"></a><span data-ttu-id="05948-102">Передача данных в столбец FILESTREAM с использованием интерфейса ISequentialStream с привязкой к параметру ICommandText (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="05948-102">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter (OLE DB)</span></span>
  <span data-ttu-id="05948-103">В этом образце используется интерфейс ISequentialStream, связанный с параметром ICommandText, для передачи от 4 МБ до 4 ГБ данных в столбец файлового потока.</span><span class="sxs-lookup"><span data-stu-id="05948-103">This sample uses an ISequentialStream interface bound to an ICommandText parameter to send between 4MB and 4GB of data to a filestream column.</span></span>  
  
 <span data-ttu-id="05948-104">Дополнительные сведения о компоненте FILESTREAM см. в разделе [Поддержка filestream &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span><span class="sxs-lookup"><span data-stu-id="05948-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="05948-105">Пример</span><span class="sxs-lookup"><span data-stu-id="05948-105">Example</span></span>  
 <span data-ttu-id="05948-106">Прежде чем переходить к компиляции и запуску этого примера, включите поддержку FILESTREAM ([включение и настройка FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span><span class="sxs-lookup"><span data-stu-id="05948-106">Before you compile and run this sample, enable FILESTREAM support ([Enable and Configure FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span></span>  
  
 <span data-ttu-id="05948-107">Убедитесь, что переменная среды INCLUDE включает каталог, содержащий файл sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="05948-107">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="05948-108">На сервере должен существовать каталог C:\DBFsa, в котором образец создаст базу данных.</span><span class="sxs-lookup"><span data-stu-id="05948-108">The server must have a directory called C:\DBFsa; this is where the sample will create the database.</span></span> <span data-ttu-id="05948-109">Экземпляр [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] должен иметь доступ на запись к этой папке (например, войдите в систему с учетной записью Local System).</span><span class="sxs-lookup"><span data-stu-id="05948-109">Your instance of [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] must have write access to this location (for example, log on as a local system account).</span></span>  
  
 <span data-ttu-id="05948-110">Скопируйте первый листинг кода и вставьте его в файл ISSHelper.h.</span><span class="sxs-lookup"><span data-stu-id="05948-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="05948-111">Скопируйте второй листинг кода и вставьте его в файл ISSHelper.cpp.</span><span class="sxs-lookup"><span data-stu-id="05948-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="05948-112">Скопируйте третий листинг кода и вставьте его в файл ICommandUpload.cpp.</span><span class="sxs-lookup"><span data-stu-id="05948-112">Copy the third code listing and paste it into a file called ICommandUpload.cpp.</span></span>  
  
 <span data-ttu-id="05948-113">Скомпилируйте файлы ICommandUpload.cpp, ISSHelper.cpp, ole32.lib и oleaut32.lib.</span><span class="sxs-lookup"><span data-stu-id="05948-113">Compile ICommandUpload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="05948-114">При запуске этого образца необходимо передать имя сервера или сервер\имя_экземпляра, а также значение между 4 МБ (0x400001) и 4 ГБ (0xFFFFFFFF), указывающее количество записываемых данных.</span><span class="sxs-lookup"><span data-stu-id="05948-114">When you run this sample, you must pass the name of a server, or a server\instance_name, as well as a value between 4 MB (0x400001) and 4 GB (0xFFFFFFFF) indicating the amount of data to write.</span></span>  
  
 <span data-ttu-id="05948-115">Четвертый листинг кода ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) удаляет базу данных, созданную в данном образце.</span><span class="sxs-lookup"><span data-stu-id="05948-115">The fourth ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) code listing deletes the database created by this sample.</span></span>  
