...
ms.locfileid: "87729473"
---
# <a name="read-a-filestream-column-to-file-using-ibcpsession-ole-db"></a><span data-ttu-id="f825f-102">Чтение столбца FILESTREAM в файле с использованием интерфейса IBCPSession (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="f825f-102">Read a FILESTREAM Column to File Using IBCPSession (OLE DB)</span></span>
  <span data-ttu-id="f825f-103">Этот образец считывает столбец файлового потока в файл с помощью интерфейса IBCPSession и записывает файл форматирования.</span><span class="sxs-lookup"><span data-stu-id="f825f-103">This sample reads a filestream column to a file using the IBCPSession interface and writes a format file.</span></span>  
  
 <span data-ttu-id="f825f-104">Дополнительные сведения о компоненте FILESTREAM см. в разделе [Поддержка filestream &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span><span class="sxs-lookup"><span data-stu-id="f825f-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="f825f-105">Пример</span><span class="sxs-lookup"><span data-stu-id="f825f-105">Example</span></span>  
 <span data-ttu-id="f825f-106">Убедитесь, что переменная среды INCLUDE включает каталог, содержащий файл sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="f825f-106">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="f825f-107">С помощью одного из следующих образцов создайте таблицу, из которой образец считывает данные.</span><span class="sxs-lookup"><span data-stu-id="f825f-107">Use one of the following samples to create the table that this sample reads from:</span></span>  
  
-   [<span data-ttu-id="f825f-108">Передача данных в столбец FILESTREAM с использованием интерфейса ISequentialStream с привязкой к параметру ICommandText &#40;OLE DB&#41;</span><span class="sxs-lookup"><span data-stu-id="f825f-108">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter &#40;OLE DB&#41;</span></span>](send-data-to-filestream-isequentialstream-bound-to-icommandtext.md)  
  
-   [<span data-ttu-id="f825f-109">Передача данных в столбец FILESTREAM с использованием интерфейса IRowsetFastUpload &#40;OLE DB&#41;</span><span class="sxs-lookup"><span data-stu-id="f825f-109">Send Data to a FILESTREAM Column Using IRowsetFastUpload &#40;OLE DB&#41;</span></span>](send-data-to-a-filestream-column-using-irowsetfastupload-ole-db.md)  
  
 <span data-ttu-id="f825f-110">Скопируйте первый листинг кода и вставьте его в файл ISSHelper.h.</span><span class="sxs-lookup"><span data-stu-id="f825f-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="f825f-111">Скопируйте второй листинг кода и вставьте его в файл ISSHelper.cpp.</span><span class="sxs-lookup"><span data-stu-id="f825f-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="f825f-112">Скопируйте третий листинг кода и вставьте его в файл IBCPSession.cpp.</span><span class="sxs-lookup"><span data-stu-id="f825f-112">Copy the third code listing and paste it into a file called IBCPSession.cpp.</span></span>  
  
 <span data-ttu-id="f825f-113">Скомпилируйте файлы IBCPSession.cpp, ISSHelper.cpp, ole32.lib и oleaut32.lib.</span><span class="sxs-lookup"><span data-stu-id="f825f-113">Compile IBCPSession.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="f825f-114">При запуске этого образца необходимо передать имя сервера (или сервер\имя_экземпляра) и имя для создаваемого файла форматирования.</span><span class="sxs-lookup"><span data-stu-id="f825f-114">When this sample is run, you must pass a server name (or server\instance_name) and a name for the format file it will create.</span></span>  
