...
ms.locfileid: "87658690"
---
# <a name="send-data-to-a-filestream-column-using-irowsetfastupload-ole-db"></a><span data-ttu-id="a34be-102">Передача данных в столбец FILESTREAM с использованием интерфейса IRowsetFastUpload (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="a34be-102">Send Data to a FILESTREAM Column Using IRowsetFastUpload (OLE DB)</span></span>
  <span data-ttu-id="a34be-103">Этот образец использует интерфейс IRowsetFastUpload, чтобы передать от 4 МБ до 4 ГБ данных в столбец файлового потока.</span><span class="sxs-lookup"><span data-stu-id="a34be-103">This sample uses the IRowsetFastUpload interface to send between 4MB and 4GB of data to a filestream column.</span></span>  
  
 <span data-ttu-id="a34be-104">Дополнительные сведения о компоненте FILESTREAM см. в разделе [Поддержка filestream &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span><span class="sxs-lookup"><span data-stu-id="a34be-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="a34be-105">Пример</span><span class="sxs-lookup"><span data-stu-id="a34be-105">Example</span></span>  
 <span data-ttu-id="a34be-106">Прежде чем переходить к компиляции и запуску этого примера, включите поддержку FILESTREAM ([включение и настройка FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span><span class="sxs-lookup"><span data-stu-id="a34be-106">Before you compile and run this sample, enable FILESTREAM support ([Enable and Configure FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span></span>  
  
 <span data-ttu-id="a34be-107">Убедитесь, что переменная среды INCLUDE включает каталог, содержащий файл sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="a34be-107">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="a34be-108">На сервере должен существовать каталог C:\DBFsa, в котором образец создаст базу данных.</span><span class="sxs-lookup"><span data-stu-id="a34be-108">The server must have a directory called C:\DBFsa; this is where the sample will create the database.</span></span> <span data-ttu-id="a34be-109">Экземпляр [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] должен иметь доступ на запись к этой папке (например, войдите в систему с учетной записью Local System).</span><span class="sxs-lookup"><span data-stu-id="a34be-109">Your instance of [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] must have write access to this location (for example, log on as a local system account).</span></span>  
  
 <span data-ttu-id="a34be-110">Скопируйте первый листинг кода и вставьте его в файл ISSHelper.h.</span><span class="sxs-lookup"><span data-stu-id="a34be-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="a34be-111">Скопируйте второй листинг кода и вставьте его в файл ISSHelper.cpp.</span><span class="sxs-lookup"><span data-stu-id="a34be-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="a34be-112">Скопируйте третий листинг кода и вставьте его в файл IRowsetFastLoadUpload.cpp.</span><span class="sxs-lookup"><span data-stu-id="a34be-112">Copy the third code listing and paste it into a file called IRowsetFastLoadUpload.cpp.</span></span>  
  
 <span data-ttu-id="a34be-113">Скомпилируйте файлы IRowsetFastLoadUpload.cpp, ISSHelper.cpp, ole32.lib и oleaut32.lib.</span><span class="sxs-lookup"><span data-stu-id="a34be-113">Compile IRowsetFastLoadUpload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="a34be-114">При запуске этого образца необходимо передать имя сервера или сервер\имя_экземпляра, а также значение между 4 МБ (0x400001) и 4 ГБ (0xFFFFFFFF), указывающее количество записываемых данных.</span><span class="sxs-lookup"><span data-stu-id="a34be-114">When you run this sample, you must pass the name of a server, or a server\instance_name, as well as a value between 4 MB (0x400001) and 4 GB (0xFFFFFFFF) indicating the amount of data to write.</span></span>  
  
 <span data-ttu-id="a34be-115">Четвертый листинг кода ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) удаляет базу данных, созданную в данном образце.</span><span class="sxs-lookup"><span data-stu-id="a34be-115">The fourth ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) code listing deletes the database created by this sample.</span></span>  
