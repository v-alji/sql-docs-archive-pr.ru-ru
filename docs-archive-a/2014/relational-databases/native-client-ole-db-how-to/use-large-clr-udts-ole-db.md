...
ms.locfileid: "87729438"
---
# <a name="use-large-clr-udts-ole-db"></a><span data-ttu-id="2133a-102">Использование определяемых пользователем типов больших данных CLR (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="2133a-102">Use Large CLR UDTs (OLE DB)</span></span>
  <span data-ttu-id="2133a-103">В этом образце показывается выборка строк с большими, определяемыми пользователем типами из результирующего набора.</span><span class="sxs-lookup"><span data-stu-id="2133a-103">This sample shows how to fetch rows with large user-defined types from a result set.</span></span> <span data-ttu-id="2133a-104">Дополнительные сведения об определяемых пользователем типах данных CLR см. в [этой статье](../native-client/ole-db/large-clr-user-defined-types-ole-db.md).</span><span class="sxs-lookup"><span data-stu-id="2133a-104">For more information, see [Large CLR User-Defined Types &#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md).</span></span> <span data-ttu-id="2133a-105">Данный образец работает с [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] или более поздней версией.</span><span class="sxs-lookup"><span data-stu-id="2133a-105">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span>  
  
## <a name="example"></a><span data-ttu-id="2133a-106">Пример</span><span class="sxs-lookup"><span data-stu-id="2133a-106">Example</span></span>  
 <span data-ttu-id="2133a-107">В этом образце содержится два проекта.</span><span class="sxs-lookup"><span data-stu-id="2133a-107">This sample contains two projects.</span></span> <span data-ttu-id="2133a-108">Один проект создает сборку (библиотеку DLL) из исходного кода на C#.</span><span class="sxs-lookup"><span data-stu-id="2133a-108">One project creates an assembly (DLL) from C# source code.</span></span> <span data-ttu-id="2133a-109">Эта сборка содержит тип CLR.</span><span class="sxs-lookup"><span data-stu-id="2133a-109">This assembly contains the CLR type.</span></span> <span data-ttu-id="2133a-110">В базу данных будет добавлена таблица.</span><span class="sxs-lookup"><span data-stu-id="2133a-110">A table will be added to the database.</span></span> <span data-ttu-id="2133a-111">Столбец в этой таблице будет иметь тип, определенный в сборке. По умолчанию в этом образце используется база данных master.</span><span class="sxs-lookup"><span data-stu-id="2133a-111">A column in the table will be of a type defined in the assembly, By default, this sample will use the master database.</span></span> <span data-ttu-id="2133a-112">Второй проект является собственным приложением C, которое считывает данные из таблицы.</span><span class="sxs-lookup"><span data-stu-id="2133a-112">The second project is a native C application that reads data from the table.</span></span>  
  
 <span data-ttu-id="2133a-113">Скомпилируйте первый листинг кода (C#) в DLL-библиотеку.</span><span class="sxs-lookup"><span data-stu-id="2133a-113">Compile the first (C#) code listing to a DLL.</span></span>  <span data-ttu-id="2133a-114">Затем скопируйте этот DLL-файл в корневой каталог диска C.</span><span class="sxs-lookup"><span data-stu-id="2133a-114">Then, copy the DLL to the root directory of the C drive.</span></span>  
  
 <span data-ttu-id="2133a-115">Выполните второй листинг кода ([!INCLUDE[tsql](../../includes/tsql-md.md)]), чтобы добавить сборку в базу данных master.</span><span class="sxs-lookup"><span data-stu-id="2133a-115">Execute the second ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to add the assembly to the master database.</span></span>  
  
 <span data-ttu-id="2133a-116">Скомпилируйте с библиотеками ole32.lib и oleaut32.lib и выполните третий листинг кода (C++).</span><span class="sxs-lookup"><span data-stu-id="2133a-116">Compile with ole32.lib oleaut32.lib and execute the third (C++) code listing.</span></span> <span data-ttu-id="2133a-117">Это приложение соединяется с установленным на компьютер экземпляром [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] по умолчанию.</span><span class="sxs-lookup"><span data-stu-id="2133a-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="2133a-118">В некоторых операционных системах Windows придется заменить (localhost) или (local) на имя своего экземпляра [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .</span><span class="sxs-lookup"><span data-stu-id="2133a-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="2133a-119">Чтобы подключиться к именованному экземпляру, измените строку подключения с L"(local)" на L"(local)\\\<имя>", где <имя> — это именованный экземпляр.</span><span class="sxs-lookup"><span data-stu-id="2133a-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="2133a-120">По умолчанию [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express устанавливается на именованный экземпляр.</span><span class="sxs-lookup"><span data-stu-id="2133a-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="2133a-121">Убедитесь, что переменная среды INCLUDE включает каталог, содержащий файл sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="2133a-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="2133a-122">Выполните четвертый листинг кода ([!INCLUDE[tsql](../../includes/tsql-md.md)]), чтобы удалить сборку из базы данных master.</span><span class="sxs-lookup"><span data-stu-id="2133a-122">Execute the fourth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to delete the assembly to the master database.</span></span>  
