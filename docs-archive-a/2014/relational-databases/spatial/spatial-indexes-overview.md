...
-   *Тесселяция геометрической сетки*, которая является схемой для `geometry` типа данных.

-   *Тесселяция географической сетки*, которая применяется к столбцам `geography` типа данных.
...
>  Эти координаты задаются предложением BOUNDING_BOX инструкции [CREATE ПРОСТРАНСТВЕННОГО индекса](/sql/t-sql/statements/create-spatial-index-transact-sql) [!INCLUDE[tsql](../../../includes/tsql-md.md)] .
 ...
 Координаты `(` _x-min_**,**_y-min_ `)` и `(` _x-max_**,**_y-max_ `)` определяют размещение и размеры ограничивающего прямоугольника. Пространство за пределами ограничивающего прямоугольника считается одной ячейкой с номером 0.
