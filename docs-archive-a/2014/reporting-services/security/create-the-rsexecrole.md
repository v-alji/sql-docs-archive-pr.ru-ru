...
5.  Введите имя экземпляра сервера компонента Database Engine. Если базы данных сервера отчетов присоединены к именованному экземпляру, необходимо ввести имя экземпляра в следующем формате: \<servername> \\<instanceName \> .
