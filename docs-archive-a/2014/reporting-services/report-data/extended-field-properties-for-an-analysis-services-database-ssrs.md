...
|`UniqueName`|`String`|Возвращает полное имя уровня. Например, `UniqueName` значением для сотрудника может быть *[Employee]. [ Отдел сотрудников]. [Department]. & [продажи]. & [менеджер по продажам в Северной Америке]. & [272]*.|  
...
|`FormattedValue`|`String`|Возвращает форматированное значение для меры или ключевую цифру. Например, `FormattedValue` свойство **квоты суммы продаж** возвращает денежный формат, например $1 124 400,00.|  
