...
|**Компонент интеграции сервера отчетов**|Включает широкие возможности создания отчетов с помощью [!INCLUDE[msCoName](../includes/msconame-md.md)][!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]<br /><br /> Этот компонент активен по умолчанию.|  
