...
## <a name="reports-with-custom-modules"></a>Отчеты с пользовательскими модулями  
 Если [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] в модулях содержится пользовательский код, он не преобразуется. Если конструктор отчетов встречает код во время импорта, создается предупреждение, которое отображается в окне **список задач** .  
...
|||||  
|-|-|-|-|  
|Изображение|Метка|Строка|Прямоугольник|  
