...
|**DocMap**|Показывает, отображается ли схема документа отчета. По умолчанию этот параметр имеет значение `true`.|  
...
|`LinkTarget`|Целевой объект гиперссылок в отчете. Можно ориентироваться на окно или фрейм, предоставив имя окна, например `LinkTarget` = *window_name*, или можно выбрать новое окно с помощью параметра `LinkTarget` = _blank. Другими допустимыми именами целевых объектов являются _self, _parent и _top.|  
