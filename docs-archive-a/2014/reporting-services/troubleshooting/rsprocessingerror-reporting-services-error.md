...
-   Параметр, у которого свойство Nullable установлено в, `False` обнаружило значение NULL в параметре.  
...
-   Если запуск производится от лица локального администратора на сервере отчетов, просмотрите файл журнала в поисках `ReportProcessingException`. В записях журнала содержится дополнительная информация. Файл журнала сервера отчетов обычно находится в папке \<*drive*> : \Program FILES\MICROSOFT SQL Server\MSRS12. Мссклсервер\репортинг Сервицес\логфилес\ ReportServerService__*датетиместамп*. log. Дополнительные сведения см. в разделе [Файлы и источники журналов служб Reporting Services](../report-server/reporting-services-log-files-and-sources.md).  
...
-   *\<report item type>* *\<report item name>* Не удается выполнить обработку для. Невозможно сравнить данные типов *\<type>* и *\<type>* . Проверьте тип данных, возвращаемый *\<report item name>* .  
  
-   Не удалось вычислить *\<property name>* .  
  
-   Не удалось вычислить *\<property name>* . Он ссылается на поле набора данных, содержащее ошибку: *\<error string>* .  
...
-   " *\<report item type>* *\<report item name>* " Имеет недопустимую область " *\<scope name>* ". Область должна совпадать с текущей или содержаться в ней.  
  
-   *\<property name>* Выражение для *\<report item type>* " *\<report item name>* " имеет параметр области, недопустимый для агрегатной функции. Параметр области должен быть задан строковой константой, совпадающей с именем объемлющей группы, объемлющей области данных или имени набора данных.  
...
-   `Previous``RunningValue`или `RowNumber` агрегатные функции, используемые в ячейках данных *\<report item type>* "", *\<report item name>* ссылаются на группирование областей как в столбцах, так и в строках *\<report item type>* . Параметры области действия All `Previous` `RunningValue` и `RowNumber` агрегатные функции в *\<report item type>* могут ссылаться на группирование строк или Группирование столбцов данных, но не на оба.  
