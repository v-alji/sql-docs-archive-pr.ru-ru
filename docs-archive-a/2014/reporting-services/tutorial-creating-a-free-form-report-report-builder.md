...
4.  Убедитесь, что тип соединения — Microsoft SQL Server, а затем в поле **строка подключения** введите: **Data Source = \<servername> **

     \<servername>Например, Report001 указывает компьютер, на котором установлен экземпляр SQL Server ядро СУБД. Поскольку данные отчета не извлекаются из базы данных SQL Server, не нужно указывать имя базы данных. Для синтаксического анализа запроса используется база данных по умолчанию на указанном сервере.
