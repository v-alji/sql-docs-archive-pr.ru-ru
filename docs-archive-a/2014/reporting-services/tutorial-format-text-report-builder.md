...
10. Выберите пустую ячейку под `[LinkText]` ячейкой, а затем нажмите и удерживайте клавишу Shift и выберите две ячейки справа: ячейка **итога** в столбце **продукт** и `[Sum(Sales)]` ячейка в столбце **продажи** .  
