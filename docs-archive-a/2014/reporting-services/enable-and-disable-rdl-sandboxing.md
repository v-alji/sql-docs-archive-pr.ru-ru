...
-   Пользовательский код в **\<Code>** элементе определения отчета.  
...
|Параметр|Описание|  
|-------------|-----------------|  
|**MaxExpressionLength**|Максимально допустимое число символов в выражении языка определения отчетов.<br /><br /> Значение по умолчанию: 1000|  
|**MaxResourceSize**|Максимально допустимый размер внешнего ресурса (КБ).<br /><br /> По умолчанию — 100|  
|**MaxStringResultLength**|Максимально допустимое число символов для возвращаемого значения выражения языка определения отчетов.<br /><br /> Значение по умолчанию: 1000|  
...
|`AllowNew`|Логический атрибут для **allow** , определяющий, разрешено ли создание новых экземпляров типа в выражениях языка определения отчетов или в элементе языка определения отчетов **\<Class>** .<br /><br /> Примечание. Если `RDLSandboxing` включен, новые массивы не могут быть созданы в выражениях языка определения отчетов, независимо от значения параметра `AllowNew` .|  
|**Значение**|Значение элемента **Allow** , представляющее имя типа, разрешенного в выражениях языка определения отчетов. Значение **\*** указывает, что разрешены все типы в пространстве имен. Это свойство учитывает регистр символов.|  
|**Члены**|Для списка типов, включенных в **\<Types>** элемент, список имен членов, недопустимых в выражениях языка определения отчетов.|  
...
-   **\<Classes>** Элемент в определении отчета.  
...
-   Добавьте **\<Deny>** элемент для имени члена.  
...
-   добавить этот новый класс к списку разрешений.  
 ...
 Чтобы добавить ключевые слова, определяющие тип [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, добавьте к списку разрешений соответствующий тип CLR. Например, чтобы использовать [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ключевое слово .NET Framework `Integer` , добавьте следующий фрагмент XML в **\<RDLSandboxing>** элемент:  
...
#### <a name="maintaining-the-deny-list-of-members"></a>Обслуживание \<Deny> списка членов  
...
### <a name="working-with-operators-and-new"></a>Работа с операторами и оператором New  
 По умолчанию все операторы языка [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, кроме `New`, всегда разрешены. `New`Оператор управляется `AllowNew` атрибутом **\<Allow>** элемента. Всегда разрешены другие операторы языка, такие как оператор метода доступа коллекции по умолчанию `!` и [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Frameworkные макросы приведения `CInt` .  
