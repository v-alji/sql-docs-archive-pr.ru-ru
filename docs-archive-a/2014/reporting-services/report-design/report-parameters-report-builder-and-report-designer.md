...
2.  ** @StartDate @EndDate Параметры и** являются параметрами @StartDate типа данных `DateTime` . Рядом с текстовым полем отображается запрос на ввод начальной даты (Start Date). Чтобы изменить дату, введите новую дату в текстовом поле или используйте элемент управления «календарь».  
...
4.  ** @ShowAllRows параметр** . параметр @ShowAllRows имеет тип данных `Boolean` . Воспользуйтесь переключателями, чтобы выбрать значение `True` или `False`.  
