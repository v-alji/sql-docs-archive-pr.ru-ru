...
### <a name="bookmarks"></a>Закладки  
 Ссылки на закладки отображаются в виде гиперссылок. Цели закладок отображаются, и к ним можно перемещаться, щелкнув ссылки закладок. При щелчке ссылки закладки отчет переходит на первое вхождение целевой метки закладки, и браузер по возможности прокручивает окно так, чтобы ссылка закладки оказалась в его верхней части. Теги привязки HTML ( \<a> ) используются для пометки целевых объектов закладок.  
...
### <a name="hyperlinks-and-drillthrough"></a>Гиперссылки и детализация  
 Гиперссылки и ссылки детализации подготавливаются к просмотру как гиперссылки на элементах отчета с помощью тегов привязки HTML ( \<a> ) вокруг элемента, для которого они определены.  
