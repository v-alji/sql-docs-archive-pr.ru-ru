...
# <a name="tutorial-add-a-sparkline-to-your-report-report-builder"></a>Учебник. Добавление спарклайна в отчет (построитель отчетов)
 ...
 Видеоролик [Как создать спарклайн в таблице (видеоматериалы по построителю отчетов)](https://technet.microsoft.com/bi/ff871942.aspx) показывает, как создать подобный отчет со спарклайнами.  
...
##  <a name="what-you-will-learn"></a><a name="BackToTop"></a>Что вы узнаете  
 ...
 2. [Создание запроса в мастере таблиц или матриц](#Query)  
...
### <a name="other-optional-steps"></a>Другие дополнительные шаги  
 ...
 6. [Форматирование данных в формате даты](#FormatDates)  
...
##  <a name="1-create-a-report-with-a-table"></a><a name="CreateTable"></a>1. Создание отчета с таблицей  
...
##  <a name="2-create-a-query-in-the-table-wizard"></a><a name="Query"></a>2. Создание запроса в мастере таблиц  
...
2.  Нажмите кнопку **изменить как текст**. Текстовый конструктор запросов отображает панель запросов и панель результатов.  
...
10. На странице **Выбор макета** в разделе **Параметры**убедитесь, что выбран параметр **Показывать подытоги и общие итоги** .  
...
9. Нажмите кнопку **Выполнить** для предварительного просмотра отчета.  
     ...
     Также обратите внимание, что для каждой строки самый высокий столбец имеет высоту строки. Это тоже верно, так как наибольшие значения для каждой строки не равны: наибольшее значение для бюджетного фильма — $10 400, но наибольшее значение для малогабаритного цифрового — $26 576 — больше, чем в два раза больше. Тем не менее наибольшие столбцы в этих двух строках имеют примерно одинаковую высоту. Ее также необходимо привести в соответствие по масштабу с другими спарклайнами.  
...
##  <a name="4-align-the-sparklines-vertically-and-horizontally"></a><a name="AlignSparklines"></a>4. Выравнивание спарклайнов по вертикали и горизонтали  
 Спарклайны трудно читать, если они не используют одинаковые измерения. И горизонтальные и вертикальные оси должны совпадать с остальными.  
...
##  <a name="5-optional-format-data-as-currency"></a><a name="FormatCurrency"></a>5. (необязательно) форматирование данных в денежном формате  
 По умолчанию сводные данные для поля **Sales (продажи** ) отображаются в виде общего числа. Отформатируйте его для представления валюты. Чтобы форматируемые текстовые поля и текст заполнителей отображался в виде образцов значений, переключайте параметр **Стили заполнителя** .  
...
##  <a name="6-optional-format-data-as-dates"></a><a name="FormatDates"></a>6. (необязательно) форматирование данных в виде дат  
 По умолчанию в поле **SalesDate** отображаются сведения о дате и времени. Можно отформатировать его таким образом, чтобы отображалась только дата.  
...
##  <a name="7-optional-change-column-widths"></a><a name="Width"></a>7. изменение ширины столбцов (необязательно)  
