...
|Missing|0|  
 ...
 Это распределение показывает, что приблизительно половина клиентов приобрела велосипед, а другая половина — нет. Этот конкретный набор данных является очень качественным, поэтому для каждого варианта в столбце [Bike Buyer] предусмотрено значение, а количество значений, помеченных как `Missing` (отсутствует), равно 0. Однако если в поле [Bike Buyer] в каком-либо из вариантов имелось значение null, [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] будет считать, что эта строка является вариантом со `Missing` значением.  
