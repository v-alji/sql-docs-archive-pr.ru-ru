...
## <a name="selecting-dimensions"></a>Выбор измерений  
 ...
 Чтобы добавить существующие измерения, выберите одно или несколько измерений в списке **Общие измерения** и нажмите кнопку со стрелкой вправо (**>**), чтобы переместить их в список **Измерения куба** . Нажмите кнопку с двойной стрелкой ( **>>** ), чтобы переместить все измерения в списке.  
...
|**Статистической обработки**|Стандартная статистическая функция для меры. Щелкните эту ячейку, чтобы задать для меры одну из стандартных статистических функций (или значение **Нет**). Значение по умолчанию при создании меры — `Sum`. В этом столбце задается свойство `AggregationFunction` объекта меры.|  
...
## <a name="defining-new-dimensions"></a>Определение новых измерений  
 ...
 Добавить измерения в куб можно с помощью сетки в окне **Добавление новых измерений**. Чтобы добавить измерение, щелкните ячейку в `Name` столбце, содержащем текст **Добавить новое измерение**, а затем введите имя измерения. Чтобы удалить строку из списка, щелкните значок удаления (**X**).  
...
|Столбец|Описание|  
|------------|-----------------|  
|**Type**|Отображает тип измерения для измерения шаблона. Щелкните эту ячейку, чтобы изменить тип измерения для измерения. Этот столбец задает свойство **Type** для объекта измерения.|  
...
|Отчетный (маркетинговый) календарь|Календарь отчетности на 12 месяцев, в котором два месяца по четыре недели и один месяц из пяти недель повторяются в трехмесячном (квартальном) шаблоне. Если выбрать этот календарь, укажите день и месяц начала, а также 3-месячный шаблон 4-4-5, 4-5-4 или 5-4-4 недель, где каждая цифра соответствует количеству недель в месяце.|  
