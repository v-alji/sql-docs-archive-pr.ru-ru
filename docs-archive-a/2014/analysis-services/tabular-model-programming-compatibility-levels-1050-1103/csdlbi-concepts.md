...
# <a name="csdlbi-concepts"></a>Основные понятия CSDLBI
 ...
 Примеры, которые используются для иллюстрации этих основных понятий, взяты из образца базы данных AdventureWorks, доступном на узле Codeplex. Дополнительные сведения о примерах см. в разделе [примеры Adventure Works для SQL Server](https://go.microsoft.com/fwlink/?linkID=220093).  
...
-   Элемент `Measure` определяет вычисления, которые могут использоваться в модели. Меру можно преобразовать в ключевой показатель эффективности, добавив набор специальных атрибутов вывода с помощью нового `KPI` элемента.  
 ...
 **Измерения:** Измерение представлено в CSDLBI как сущность (таблица) со столбцами и свойствами. Обратите внимание, что измерение, даже не входящее в перспективу, но включенное в модель, будет представлено в выходе языка CSDL, отмеченное как `Hidden`.  
  
 **Перспективы:** Клиент может запрашивать язык CSDL для отдельных перспектив. Дополнительные сведения см. в разделе [DISCOVER_CSDL_METADATA набор строк](https://docs.microsoft.com/bi-reference/schema-rowsets/xml/discover-csdl-metadata-rowset).  
  
 **Иерархии:** Иерархии поддерживаются и представлены в CSDLBI как набор уровней.  
  
 **Элементы:** Добавлена поддержка элемента по умолчанию, и значения по умолчанию автоматически добавляются в выходные данные CSDLBI.  
  
 **Вычисляемые элементы:** Многомерные модели поддерживают вычисляемые элементы для дочерних элементов **с одним** реальным элементом.  
  
 **Атрибуты измерения:** В выходных данных CSDLBI атрибуты измерения поддерживаются и автоматически помечаются как не подлежащие статистической обработке.  
 ...
 **Новые свойства:** Добавлены дополнительные атрибуты для поддержки моделей DirectQuery.  
