...
---
# <a name="hierarchy-representation-tabular"></a><span data-ttu-id="ae48b-102">Представление иерархии (табличное)</span><span class="sxs-lookup"><span data-stu-id="ae48b-102">Hierarchy Representation (Tabular)</span></span>
  <span data-ttu-id="ae48b-103">В табличных моделях иерархия представляет собой путь навигации от одного атрибута к другому в зависимости от значений, выбранных пользователем.</span><span class="sxs-lookup"><span data-stu-id="ae48b-103">In tabular models a hierarchy is navigation path from one attribute to another based on values selected by the user.</span></span>  
  
## <a name="hierarchy-representation"></a><span data-ttu-id="ae48b-104">Представление иерархии</span><span class="sxs-lookup"><span data-stu-id="ae48b-104">Hierarchy Representation</span></span>  
  
### <a name="hierarchy-in-amo"></a><span data-ttu-id="ae48b-105">Иерархия в объектах AMO</span><span class="sxs-lookup"><span data-stu-id="ae48b-105">Hierarchy in AMO</span></span>  
 <span data-ttu-id="ae48b-106">Если для управления таблицей табличной модели используются объекты AMO, то для иерархии объектов в AMO существует идентичный «один к одному» объект.</span><span class="sxs-lookup"><span data-stu-id="ae48b-106">When using AMO to manage a tabular model table, there is a one-to-one object match for a hierarchy in AMO.</span></span> <span data-ttu-id="ae48b-107">Иерархия представляется объектом <xref:Microsoft.AnalysisServices.Hierarchy>.</span><span class="sxs-lookup"><span data-stu-id="ae48b-107">A hierarchy is represented by the <xref:Microsoft.AnalysisServices.Hierarchy> object.</span></span>  
  
 <span data-ttu-id="ae48b-108">В следующем фрагменте кода показано, как добавить иерархию в существующую табличную модель.</span><span class="sxs-lookup"><span data-stu-id="ae48b-108">The following code snippet shows how to add a hierarchy to an existing tabular model.</span></span> <span data-ttu-id="ae48b-109">В коде предполагается, что имеется объект базы данных AMO с именем newDatabase и объект куба AMO с именем modelCube.</span><span class="sxs-lookup"><span data-stu-id="ae48b-109">The code assumes you have an AMO database object, newDatabase, and an AMO cube object, modelCube.</span></span>  
