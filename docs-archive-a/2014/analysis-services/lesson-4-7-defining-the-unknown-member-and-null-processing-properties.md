...
5.  Перейдите на вкладку **браузер** , убедитесь в том, что в списке **Иерархия** выбран пункт **линии модели продукта** , а затем разверните `All Products` .
...
17. Укажите в `All Products` качестве значения свойства **AllMemberName** пользовательской иерархии категории продуктов.
...
5.  В диалоговом окне **Создание связи атрибутов** в поле **Исходный атрибут** имеет значение `Subcategory` . Задайте для параметра **связанный атрибут** значение `Category` . Оставьте для типа связи значение **Гибкая**.
...
3.  Убедитесь, что в списке **Иерархия** выбраны **категории продуктов** , а затем разверните `All Products` .
