...
6.  В диалоговом окне **Выбор столбцов** проверьте список столбцов и при необходимости измените использование столбцов на одно из следующих значений:  
    ...
    -   **Вход.** Указывает, что столбец содержит переменные, которые влияют на результат и должны использоваться в качестве входных данных для модели.  
    ...
    -   **Key**. Для каждой модели требуется по крайней мере один ключ. В зависимости от типа модели также можно использовать дополнительные специальные ключи, например **секуенцекэй** или **TimeKey**.  
