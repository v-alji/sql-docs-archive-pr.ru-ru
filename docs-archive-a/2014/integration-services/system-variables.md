...
|**InteractiveMode**|Логическое|Указывает, выполняется ли пакет в интерактивном режиме. Если пакет выполняется в конструкторе служб [!INCLUDE[ssIS](../includes/ssis-md.md)], это свойство равно `True`. Если пакет запускается с помощью программы командной строки **dtexec** , свойство имеет значение `False` .|  
...
|**ErrorDescription**|Строка|Описание ошибки.|Обработчик события OnError<br /><br /> Обработчик события OnInformation<br /><br /> Обработчик события OnWarning|  
...
|`Propagate`|Логическое|Указывает, будет ли событие передаваться обработчику события верхнего уровня.<br /><br /> Примечание. Значение переменной `Propagate` не учитывается при проверке пакета.<br /><br /> Если в дочернем пакете свойству `Propagate` присвоено значение `False`, это не препятствует распространению события в родительский пакет.|Все обработчики событий|  
