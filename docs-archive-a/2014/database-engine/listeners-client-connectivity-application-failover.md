...
##  <a name="availability-group-listeners"></a><a name="AGlisteners"></a>Прослушиватели группы доступности  
...
###  <a name="availability-group-listener-configuration"></a><a name="AGlConfig"></a> Конфигурация прослушивателя группы доступности  
...
###  <a name="selecting-an-availability-group-listener-port"></a><a name="SelectListenerPort"></a> Выбор порта прослушивателя группы доступности  
...
##  <a name="using-a-listener-to-connect-to-the-primary-replica"></a><a name="ConnectToPrimary"></a>Использование прослушивателя для подключения к первичной реплике  
...
##  <a name="using-a-listener-to-connect-to-a-read-only-secondary-replica-read-only-routing"></a><a name="ConnectToSecondary"></a> Соединение с помощью прослушивателя со вторичной репликой только для чтения (маршрутизация только для чтения)  
...
###  <a name="to-configure-availability-replicas-for-read-only-routing"></a><a name="ConfigureARsForROR"></a> Настройка реплик доступности для маршрутизации только для чтения  
...
####  <a name="related-tasks"></a><a name="RelatedTasksROR"></a> Связанные задачи  
...
###  <a name="read-only-application-intent-and-read-only-routing"></a><a name="ReadOnlyAppIntent"></a>Назначение приложения только для чтения и маршрутизация только для чтения  
...
```  
 ...
 В этом примере строки подключения клиент пытается подключиться к прослушивателю группы доступности `AGListener` через порт 1433 (номер порта также можно не указывать, если прослушиватель группы доступности отслеживает порт 1433).  Для строки подключения свойство имеет `ApplicationIntent` значение `ReadOnly` , что делает *строку подключения с намерением чтения*.  Без этого параметра сервер не стал бы пытаться выполнить для этого подключения маршрутизацию только для чтения.  
...
####  <a name="related-tasks"></a><a name="RelatedTasksApps"></a> Связанные задачи  
...
##  <a name="bypassing-availability-group-listeners"></a><a name="BypassAGl"></a>Обход прослушивателей группы доступности  
...
###  <a name="using-database-mirroring-connection-strings-with-availability-groups"></a><a name="DbmConnectionString"></a>Использование строк подключения зеркального отображения базы данных с группами доступности  
...
##  <a name="supporting-availability-group-multi-subnet-failovers"></a><a name="SupportAgMultiSubnetFailover"></a> Поддержка отказоустойчивых кластеров с несколькими подсетями группы доступности  
...
##  <a name="availability-group-listeners-and-ssl-certificates"></a><a name="SSLcertificates"></a> Прослушиватели группы доступности и SSL-сертификаты  
...
##  <a name="availability-group-listeners-and-server-principal-names-spns"></a><a name="SPNs"></a>Прослушиватели групп доступности и имена участников-серверов (SPN)  
